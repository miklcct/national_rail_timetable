...
class Timetable {
    ...
    public static function generateFromBoard(DepartureBoard $board) : static {
        ...
        usort(
            ...
            count($arrival_mode ? $a->getPrecedingPublicCalls() : $a->getSubsequentPublicCalls())
            <=> count($arrival_mode ? $b->getPrecedingPublicCalls() : $b->getSubsequentPublicCalls())
        )
        );
        $common_check = true;
            ...
            foreach ($calls as &$call) {
                if ($call !== null) {
                    ...
                    foreach (array_keys($portions) as $portion) {
                        ...
                        $found_one = false;
                        foreach ($arrival_mode ? array_reverse($call->getPrecedingPublicCalls()) : $call->getSubsequentPublicCalls() as $subsequent_call) {
                            if (
                                array_key_exists(
                                    $portion,
                        ...
                        if ($stations === []) {
                            ...
                            foreach ($preorder as $list) {
                                foreach ([$list, array_reverse($list)] as $list_direction) {
                                    ...
                                    if ($count > $max_count) {
                                        $stations = array_map(
                                            static fn(string $crs) => new class($crs) {
        ...
        foreach ($board->calls as $call) {
            foreach (array_keys($arrival_mode ? $call->service->getOriginPortions() : $call->service->getDestinationPortions()) as $portion) {
                $matrix[0][$i] = $call;
                $j = 1;
                foreach ($arrival_mode ? $call->getPrecedingPublicCalls() : $call->getSubsequentPublicCalls() as $subsequent_call) {
